...
```
987654321111111 → 98 (positions 0,1)
811111111111119 → 89 (positions 0,14)
234234234234278 → 78 (positions 13,14)
818181911112111 → 92 (positions 5,7)
Total: 357
```
...
```
987654321111111 → 987654321111 (skip last 3 ones)
811111111111119 → 811111111119 (skip middle ones)
234234234234278 → 434234234278 (skip first 2,3,2)
818181911112111 → 888911112111 (skip early ones)
Total: 3121910778619
```
...
> 🍩 **Note sur les algorithmes greedy**:
L'algorithme "greedy" (glouton en français) s'appelle ainsi parce qu'il fait toujours le choix qui semble le meilleur à l'instant présent, sans considérer les conséquences futures - un peu comme quelqu'un de gourmand qui prendrait toujours le plus gros morceau de gâteau immédiatement disponible.

Caractéristiques de l'approche "greedy":
Choix localement optimal: À chaque étape, l'algorithme prend la décision qui paraît la meilleure selon un critère donné
Pas de retour en arrière: Une fois qu'un choix est fait, il n'est jamais reconsidéré  
Myopie: L'algorithme ne regarde pas plus loin que l'étape courante

Exemples classiques:  
Rendu de monnaie: toujours rendre la pièce de plus grande valeur possible  
Problème du sac à dos fractionnaire: prendre les objets avec le meilleur rapport valeur/poids  
Algorithme de Dijkstra: explorer le nœud le plus proche  

Important: Les algorithmes greedy ne garantissent pas toujours la solution optimale globale - ils trouvent juste une solution "assez bonne" rapidement. Ils sont souvent utilisés dans l'Advent of Code quand on cherche une heuristique efficace
---
