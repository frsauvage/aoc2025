...
- Position (0,2): `@` has 2 neighbors → accessible ✓
- Position (1,0): `@` has 2 neighbors → accessible ✓
- Position (5,1): `@` has 7 neighbors → NOT accessible ✗
- Position (3,7): `.` (empty) → not a roll
...
Starting from the same 10×10 grid with 67 total rolls.

**Round 1:** Remove 13 accessible rolls (highlighted with `*`)
```
..*@.*@@*.     →     ..xx.xxxx.
*@@.@.@.@@            xxx.x.x.xx
@@@@@.*.**            xxxxx.xxxx
...
- If we need to change the criterion → must modify 2 places
...
- **DRY approach:** Change only `find_accessible()` → done! ✅
...
**Benefits:**
- ✅ `solve_part1()`: **2 lines** instead of 15
- ✅ `solve_part2()`: **8 lines** instead of 20
...
**Rule of thumb:** If you copy-paste code → create a function instead! 🎯
